...
function formatDraftSummary(draft: ListingDraft, lang: Language): string {
  ...
  lines.push(
    lang === "fr"
      ? `Loyer: ${draft.rent?.toLocaleString() ?? "?"} KES/mois`
...
async function promptAddressStep(
  ...
  if (step === "town") {
    await sendTextMessage(
      phone,
      lang === "fr"
        ? "Tapez le nom de la ville (ex: Nairobi, Mombasa, Kisumu, Nakuru)."
  ...
  if (step === "quarter") {
    await sendTextMessage(
      phone,
      lang === "fr"
        ? "Tapez le nom du quartier (ex: Westlands, Kilimani, Nyali)."
...
export async function handleLandlordListing(
  ...
  switch (session.step) {
    ...
    case "subtype": {
      ...
      await sendTextMessage(
        phone,
        lang === "fr" ? "Loyer mensuel en KES ?" : "Monthly rent in KES?"
      );
      return;
    }
